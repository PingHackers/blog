...
通过查找资料发现，基于帧的算法（Frame-based）来实现动画会导致不同帧率的平台体验不一致，后来研究了一下基于时间（Time-based）的动画算法。才发现，要实现动画，用Time-based才是王道。使用Time-based取代原来的Frame-based进行改良以后，不同平台都有了大致一致速度上的体验。
